...
const shopConfig = {};
const seasonFile = `./Config/shops/${config.ShopSeason}.json`;
let itemsJson = [];

try {
  itemsJson = JSON.parse(fs.readFileSync(seasonFile).toString());
  console.log(`Loaded shop items for ${config.ShopSeason}`);
} catch (err) {
  console.error(`Failed to load shop items for ${config.ShopSeason}:`, err);
  return;
}
...
function createShopConfig() {
  ...
  const dailyItems = eligibleItems
    .filter(item => !featuredItems.includes(item))
    .sort(() => 0.5 - Math.random())
    .slice(0, 6);
...
function sendWebhook(shopImage) {
  const webhookClient = new WebhookClient({ url: config.shop.webhookurl });

  const attachment = new MessageAttachment(shopImage, 'itemshop.png');
  const embed = new MessageEmbed()
    .setTitle('Fortnite Item Shop')
    .setDescription('Here is the current item shop!')
    .setColor('#ffffff')
    .setImage('attachment://itemshop.png')
    .setTimestamp();
  ...
  webhookClient.send({
    ...
    .then(() => console.log('Shop posted to Discord!'))
    .catch(console.error);
}
